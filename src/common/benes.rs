...
#[cfg(any(feature = "mceliece348864", feature = "mceliece348864f"))]
pub(crate) fn apply_benes_gf12(
    r: &mut [u8; 512],
    bits: &[u8; super::gf12::COND_BYTES],
    rev: usize,
) {
    use super::gf12::GFBITS;
...
/// `rev` toggles between normal application (0) or its inverse (!0).
#[cfg(not(any(feature = "mceliece348864", feature = "mceliece348864f")))]
pub(crate) fn apply_benes_gf13(
    r: &mut [u8; 1024],
    bits: &[u8; super::gf13::COND_BYTES],
    rev: usize,
) {
    use super::gf13::GFBITS;
...
mod tests {
    ...
    fn get(name: &str) -> Vec<u64> {
        crate::TestData::new().u64vec(&name)
    }
    ...
    #[test]
    fn test_layer_2() {
        let mut data_arg = get64("mceliece348864_benes_layer_data_before");
        let bits_arg = get("mceliece348864_benes_layer_bits");
        layer(&mut data_arg, &bits_arg, 0);
        let actual_data = data_arg;

        let expected_data = get64("mceliece348864_benes_layer_data_after");
    ...
    #[test]
    fn test_layer_in() {
        let data0_arg = get64("mceliece460896_benes_layer_in_data0_before");
        let data1_arg = get64("mceliece460896_benes_layer_in_data1_before");
        let mut data_arg = [data0_arg, data1_arg];
        let bits_arg = get64("mceliece460896_benes_layer_in_bits");
        layer_in(&mut data_arg, &bits_arg, 0);
        let actual_data = data_arg;

        let expected_data0 = get64("mceliece460896_benes_layer_in_data0_after");
        let expected_data1 = get64("mceliece460896_benes_layer_in_data1_after");
    ...
    fn test_apply_benes() {
        ...
        apply_benes_gf12(&mut r_arg, &bits_arg, 0);
        let actual_r = r_arg;
        let expected_r =
    ...
    fn test_apply_benes() {
        ...
        apply_benes_gf13(&mut r_arg, &bits_arg, 0);
        let actual_r = r_arg;
        let expected_r =
